  ...
  export default {
    data() {
      return {
        ...
        fields: [
           { key: 'idteacher', label: 'ID', sortable: true,  },
          { key: 'teacher_name', label: 'Name', sortable: true,  },
          { key: 'nic', label: 'NIC', sortable: true,  },
          { key: 'join_date', label: 'Date joined', sortable: true, },
    ...
    mounted(){
      const axios = require('axios'); this.isBusy = true;
        axios.get("http://localhost:8000/api/all_teacher")
            .then(response => {
