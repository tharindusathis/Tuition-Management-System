...
:busy.sync="isBusy"
    ...
    <div slot="table-busy" class="text-center text-danger my-2">
      ...
      <template slot="idteacher_payment" slot-scope="row">
        {{ row.value ? 'YES' : 'NO' }}
      </template>
  ...
  export default {
    data() {
      return {
        ...
        fields: [
          { key: 'idclass_log', label: 'ID', sortable: true },
          ...
          { key: 'idteacher_payment', label: 'Payed To Teacher', sortable: true },
          { key: 'attendace_count', label: 'Attended Count', sortable: true },
          { key: 'student_payed_count', label: 'Payed Count', sortable: true },
          // { key: '', label: '', sortable: true, sortDirection: 'desc' },
        ],
        currentPage: 1,
        perPage: 50,
        totalRows: 10,
    ...
    mounted(){
      const axios = require('axios'); this.isBusy = true;
        axios.get("http://localhost:8000/api/all_class_log")
            .then(response => {
