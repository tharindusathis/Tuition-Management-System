  ...
  export default {
    data() {
      return {
        ...
        fields: [
          { key: 'idhall', label: 'ID', sortable: true},
          { key: 'hall_name', label: 'Name', sortable: true},
          { key: 'timeslot_count', label: 'Timeslots count', sortable: true},
          { key: 'note', label: 'Note'},
        ],
        currentPage: 1,
        perPage: 10,
    ...
    mounted(){
      const axios = require('axios'); this.isBusy = true;
        axios.get("http://localhost:8000/api/all_hall")
            .then(response => {
